...
Linux artifacts are built inside Valve Steam Runtime 3 **sniper** rather than directly on the GitHub Ubuntu host. This keeps the `linuxsteamrt64` binary compatible with glibc 2.31 environments and prevents errors such as `GLIBC_2.32 not found`. The CI also checks the highest required GLIBC symbol and fails if it exceeds 2.31.
